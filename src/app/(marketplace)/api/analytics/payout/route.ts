...
export async function POST(req: Request) {
  ...
  if (!user || (user.role !== 'promoter' && user.role !== 'customer')) {
    return new NextResponse('User not found or not eligible for promoter payouts', { status: 404 });
  }
  ...
  const record = await prisma.payout.create({
    data: {
      ...
      notes: typeof note === 'string' && note.trim() ? note.trim() : 'Moderator-triggered payout',
  ...
  return NextResponse.json({ message: 'Payout marked as sent for promoter', payout: record });
}
