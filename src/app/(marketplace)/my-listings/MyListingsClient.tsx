...
const MyListingsClient: React.FC<MyListingsClientProps> = ({ listings, currentUser }) => {
  ...
  const setTabGuarded = (next: TabKey) => {
    userChangedTabRef.current = true;   // only start syncing URL after explicit user action
    setTab(next);
  };

  useEffect(() => {
    if (!userChangedTabRef.current) return; // skip initial mount to avoid first replace
